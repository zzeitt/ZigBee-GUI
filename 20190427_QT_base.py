...
class MyWinMap(QWidget):
    ...
    def initMap(self):
        ...
        try:
            ...
            # view视图鼠标响应
            self.gview_map.mousePressEvent = self.onClickedMap
            w = self.width()
            h = self.height()
            self.moveToCenter(w, h)
            # 节点相关部分
            self.node_list = []  # 存储节点的坐标信息
            ...
            if self.chb_mark.isChecked():
                ...
                if event.button() == Qt.LeftButton:
                    if len(self.node_list) < NODE_NUM:
                        self.node_list.append(self.node_pos)
                        # 绘图部分
                        self.drawIndexCircle(
                            len(self.node_list), self.node_pos)
                # 右键回退标记
                if event.button() == Qt.RightButton:
                    if len(self.node_list) > 0:
                        self.node_list.pop()
                        self.gscene_map.clear()
                        self.gscene_map.addPixmap(self.pixmap_map)
                        for i in range(len(self.node_list)):
                            self.drawIndexCircle(i+1, self.node_list[i])
                print('node_list: ', self.node_list)

    def drawIndexCircle(self, index, node_pos):
        # 样式设置
        node_draw_r = 15
        ...
        # 索引号
        self.text_index = '<div style=\"font:26px;\
